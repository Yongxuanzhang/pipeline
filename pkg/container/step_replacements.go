...
func ApplyStepReplacements(step *pipeline.Step, stringReplacements map[string]string, arrayReplacements map[string][]string) {
	step.Script = substitution.ApplyReplacements(step.Script, stringReplacements)
	step.OnError = (pipeline.OnErrorType)(substitution.ApplyReplacements(string(step.OnError), stringReplacements))
...
func ApplyStepTemplateReplacements(stepTemplate *pipeline.StepTemplate, stringReplacements map[string]string, arrayReplacements map[string][]string) {
