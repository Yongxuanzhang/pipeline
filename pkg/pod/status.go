...
func MakeTaskRunStatus(ctx context.Context, logger *zap.SugaredLogger, tr v1.TaskRun, pod *corev1.Pod, kubeclient kubernetes.Interface, ts *pipeline.TaskSpec) (v1.TaskRunStatus, error) {
	trs := &tr.Status
...
func setTaskRunStatusBasedOnStepStatus(ctx context.Context, logger *zap.SugaredLogger, stepStatuses []corev1.ContainerStatus, tr *v1.TaskRun, podPhase corev1.PodPhase, kubeclient kubernetes.Interface, ts *pipeline.TaskSpec) *multierror.Error {
	trs := &tr.Status
	var merr *multierror.Error
	...
	if tr.Spec.TaskSpec != nil {
		...
		err:=v1.Convert_v1_TaskSpec_To_pipeline_TaskSpec(tr.Spec.TaskSpec,internalts,nil)
		if err!=nil{
			merr = multierror.Append(merr, err)
		}
...
func filterResults(results []result.RunResult, specResults []pipeline.TaskResult) ([]v1.TaskRunResult, []result.RunResult) {
	var taskResults []v1.TaskRunResult
	var filteredResults []result.RunResult
	neededTypes := make(map[string]pipeline.ResultsType)
	...
	for _, r := range results {
		...
		case result.TaskRunResultType:
			...
			if neededTypes[r.Key] == pipeline.ResultsTypeString {
