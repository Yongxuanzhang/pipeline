...
func TestSetTaskRunStatusBasedOnStepStatus(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			merr := setTaskRunStatusBasedOnStepStatus(context.Background(), logger, c.ContainerStatuses, &tr, corev1.PodRunning, kubeclient, &pipeline.TaskSpec{})
			if merr != nil {
...
func TestSetTaskRunStatusBasedOnStepStatus_sidecar_logs(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			merr := setTaskRunStatusBasedOnStepStatus(ctx, logger, []corev1.ContainerStatus{{}}, &tr, pod.Status.Phase, kubeclient, &pipeline.TaskSpec{})
...
func TestMakeTaskRunStatus(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.desc, func(t *testing.T) {
			...
			logger, _ := logging.NewLogger("", "status")
			kubeclient := fakek8s.NewSimpleClientset()
			got, err := MakeTaskRunStatus(context.Background(), logger, tr, &c.pod, kubeclient, &pipeline.TaskSpec{})
			if err != nil {
...
func TestMakeRunStatusJSONError(t *testing.T) {
	...
	logger, _ := logging.NewLogger("", "status")
	kubeclient := fakek8s.NewSimpleClientset()
	gotTr, err := MakeTaskRunStatus(context.Background(), logger, tr, pod, kubeclient, &pipeline.TaskSpec{})
	if err == nil {
