...
func propagateParams(t v1.PipelineTask, stringReplacements map[string]string, arrayReplacements map[string][]string, objectReplacements map[string]map[string]string) v1.PipelineTask {
	...
	if len(t.Params) > 0 {
		...
		var internaltaskspec pipeline.TaskSpec
		v1.Convert_v1_TaskSpec_To_pipeline_TaskSpec(&t.TaskSpec.TaskSpec,&internaltaskspec,nil)
		...
		v1.Convert_pipeline_TaskSpec_To_v1_TaskSpec(&internalts,&v1taskspec,nil)
