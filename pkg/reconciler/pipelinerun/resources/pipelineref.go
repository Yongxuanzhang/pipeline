...
func verifyResolvedPipeline(ctx context.Context, pipeline v1beta1.PipelineObject) error {
	cfg := config.FromContextOrDefaults(ctx)
	if cfg.FeatureFlags.VerificationPolicy != config.SkipResourceVerificationMode && cfg.FeatureFlags.EnableAPIFields == config.AlphaAPIFields {
		if err := trustedresources.VerifyPipeline(ctx, pipeline.Copy()); err != nil {
			if cfg.FeatureFlags.VerificationPolicy == config.EnforceResourceVerificationMode {
				return err
			}
