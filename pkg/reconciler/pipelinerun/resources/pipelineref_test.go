...
func TestLocalPipelineRef_TrustedResourceVerification(t *testing.T) {
	...
	signedPipeline, err := trustedresources.GetSignedPipeline(unsignedPipeline, signer, "test-signed")
	if err != nil {
		t.Fatal("fail to sign pipeline", err)
	}

	// attack another signed pipeline
	signedPipeline2, err := trustedresources.GetSignedPipeline(internaltesting.GetUnsignedPipeline("test-pipeline2"), signer, "test-signed2")
	if err != nil {
...
func TestGetPipelineFunc_RemoteResolution_TrustedResourceVerification(t *testing.T) {
	...
	signedPipeline, err := trustedresources.GetSignedPipeline(unsignedPipeline, signer, "signed")
	if err != nil {
		t.Fatal("fail to sign pipeline", err)
	}
