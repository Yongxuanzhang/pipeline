...
func validateParams(ctx context.Context, paramSpecs []pipeline.ParamSpec, params pipeline.Params, matrixParams pipeline.Params) error {
	if paramSpecs == nil {
		return nil
	}
...
func neededParamsNamesAndTypes(paramSpecs []pipeline.ParamSpec) (sets.String, map[string]pipeline.ParamType) {
	neededParamsNames := sets.String{}
	neededParamsTypes := make(map[string]pipeline.ParamType)
...
// in the paramSpec
func missingParamsNames(neededParams sets.String, providedParams sets.String, paramSpecs []pipeline.ParamSpec) []string {
...
func wrongTypeParamsNames(params []pipeline.Param, matrix pipeline.Params, neededParamsTypes map[string]pipeline.ParamType) []string {
	...
	for _, param := range params {
		...
		if param.Value.Type == pipeline.ParamTypeString && (neededParamsTypes[param.Name] == pipeline.ParamTypeArray || neededParamsTypes[param.Name] == pipeline.ParamTypeObject) && v1.VariableSubstitutionRegex.MatchString(param.Value.StringVal) {
			continue
		}
	...
	for _, param := range matrix {
		...
		if neededParamsTypes[param.Name] != pipeline.ParamTypeString {
			wrongTypeParamNames = append(wrongTypeParamNames, param.Name)
		}
	}
...
func MissingKeysObjectParamNames(paramSpecs []pipeline.ParamSpec, params pipeline.Params) map[string][]string {
	neededKeys := make(map[string][]string)
	providedKeys := make(map[string][]string)

	for _, spec := range paramSpecs {
		if spec.Type == pipeline.ParamTypeObject {
	...
	for _, p := range params {
		if p.Value.Type == pipeline.ParamTypeObject {
...
func ValidateResolvedTask(ctx context.Context, params []pipeline.Param, matrix *pipeline.Matrix, rtr *resources.ResolvedTask) error {
	var paramSpecs pipeline.ParamSpecs
	if rtr != nil {
...
func validateTaskSpecRequestResources(taskSpec *pipeline.TaskSpec) error {
	if taskSpec != nil {
...
func validateOverrides(ts *pipeline.TaskSpec, trs *v1.TaskRunSpec) error {
...
func validateStepOverrides(ts *pipeline.TaskSpec, trs *v1.TaskRunSpec) error {
	var err error
...
func validateSidecarOverrides(ts *pipeline.TaskSpec, trs *v1.TaskRunSpec) error {
	var err error
...
func validateTaskRunResults(tr *v1.TaskRun, resolvedTaskSpec *pipeline.TaskSpec) error {
	specResults := []pipeline.TaskResult{}
	if tr.Spec.TaskSpec != nil {
		ts :=&pipeline.TaskSpec{}
		v1.Convert_v1_TaskSpec_To_pipeline_TaskSpec(tr.Spec.TaskSpec,ts, nil)
...
func mismatchedTypesResults(tr *v1.TaskRun, specResults []pipeline.TaskResult) map[string]string {
...
func missingKeysofObjectResults(tr *v1.TaskRun, specResults []pipeline.TaskResult) map[string][]string {
	neededKeys := make(map[string][]string)
	providedKeys := make(map[string][]string)
