...
func getVerifiers(ctx context.Context) ([]signature.Verifier, error) {
	...
	if cfg.TrustedResources.KMSKey != "" {
		// Fetch key from kms.
		v, err := kms.Get(ctx, cfg.TrustedResources.KMSKey, crypto.SHA256)
		if err == nil {
			verifiers = append(verifiers, v)
		}
	} else {
		v, err := cosignsignature.LoadPublicKey(ctx, cfg.TrustedResources.CosignKey)
		fmt.Println()
		if err == nil {
			verifiers = append(verifiers, v)
		}
