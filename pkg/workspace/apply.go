...
func getDeclaredWorkspace(name string, w []pipeline.WorkspaceDeclaration) (*pipeline.WorkspaceDeclaration, error) {
...
func Apply(ctx context.Context, ts pipeline.TaskSpec, wb []v1.WorkspaceBinding, v map[string]corev1.Volume) (*pipeline.TaskSpec, error) {
	...
	if ts.StepTemplate == nil {
		ts.StepTemplate = &pipeline.StepTemplate{}
	}
	...
	for i := range wb {
		...
		if addWorkspace {
			ts.Workspaces = append(ts.Workspaces, pipeline.WorkspaceDeclaration{Name: wb[i].Name})
		}
...
// a TaskSpec.
func mountAsSharedWorkspace(ts pipeline.TaskSpec, volumeMount corev1.VolumeMount) {
...
func mountAsIsolatedWorkspace(ts pipeline.TaskSpec, workspaceName string, volumeMount corev1.VolumeMount) {
...
func AddSidecarVolumeMount(sidecar *pipeline.Sidecar, volumeMount corev1.VolumeMount) {
