...
func TestApply(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		workspaces       []v1.WorkspaceBinding
		expectedTaskSpec pipeline.TaskSpec
	}{{
		name: "binding a single workspace with a PVC",
		ts: pipeline.TaskSpec{
...
func TestApply_IsolatedWorkspaces(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		ts: pipeline.TaskSpec{
			...
			Sidecars: []pipeline.Sidecar{{Name: "foo"}},
			Workspaces: []pipeline.WorkspaceDeclaration{{
				Name: "source",
			}},
		},
...
func TestApplyWithMissingWorkspaceDeclaration(t *testing.T) {
	...
	ts := pipeline.TaskSpec{
		Steps:      []pipeline.Step{{}},
		Sidecars:   []pipeline.Sidecar{{}},
		Workspaces: []pipeline.WorkspaceDeclaration{}, // Intentionally missing workspace declaration
	}
...
func TestAddSidecarVolumeMount(t *testing.T) {
	for _, tc := range []struct {
		...
		expectedSidecar pipeline.Sidecar
	}{{
		sidecarMounts: nil,
		volumeMount: corev1.VolumeMount{
			Name:      "foo",
